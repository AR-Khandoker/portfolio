...
export function WorkPage() {
  ...
  // After async projects arrive the page grows significantly.
  // Notify Lenis (via custom event) and ScrollTrigger to recalculate bounds.
  useEffect(() => {
    if (!loading) {
      requestAnimationFrame(() => {
        window.dispatchEvent(new CustomEvent('portfolio:content-loaded'));
      ...
      <div className="container mx-auto px-6">
        {loading ? (
          ...
          <div className="bg-red-50 border border-red-200 rounded-2xl p-8 text-center my-8">
            <p className="text-red-600 mb-1">Failed to load projects</p>
            <p className="text-red-400 text-sm">{error}</p>
          </div>
        ) : filteredProjects.length === 0 ? (
          <div className="bg-gray-50 border border-black/10 rounded-2xl p-12 text-center my-8">
            <p className="text-black/60 mb-1">No projects found</p>
        ...
        <div className="pt-16 border-t border-black/10">
          <AnimatedSection animation="fade">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
              ...
              <p className="text-black/55 max-w-md">
                UI snippets, experiments, and detailed case study presentations on other platforms.
              </p>
            </div>
          </AnimatedSection>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              >
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-black group-hover:text-[#ea4c89] transition-colors">
                      Dribbble
                    </h3>
                    <p className="text-black/55 text-sm">UI Snapshots & Experiments</p>
                  </div>
                </div>
                <ArrowUpRight className="w-5 h-5 text-black/25 group-hover:text-black group-hover:translate-x-1 group-hover:-translate-y-1 transition-all" />
              </a>
              ...
              >
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-black group-hover:text-[#0057ff] transition-colors">
                      Behance
                    </h3>
                    <p className="text-black/55 text-sm">Case Studies & Portfolios</p>
                  </div>
                </div>
                <ArrowUpRight className="w-5 h-5 text-black/25 group-hover:text-black group-hover:translate-x-1 group-hover:-translate-y-1 transition-all" />
              </a>
