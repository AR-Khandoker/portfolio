...
// GET all
app.get('/make-server-89f52d09/motion-videos', async (c) => {
  try {
...
// GET single
app.get('/make-server-89f52d09/motion-videos/:id', async (c) => {
  try {
    await ensureInitialized();
...
// POST create
app.post('/make-server-89f52d09/motion-videos', async (c) => {
  try {
    await ensureInitialized();
...
// PUT update
app.put('/make-server-89f52d09/motion-videos/:id', async (c) => {
  try {
    await ensureInitialized();
    const id = c.req.param('id');
...
// DELETE
app.delete('/make-server-89f52d09/motion-videos/:id', async (c) => {
  try {
    await ensureInitialized();
    const id = c.req.param('id');
    const existing = await kv.get(`motion:${id}`);
